...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   TMVA::Reader *reader_trgmu = new TMVA::Reader( "!Color:!Silent" );
   reader_trgmu->AddVariable("muPt1st", &muPt1st_TMVA);
   reader_trgmu->AddVariable("muPt2nd", &muPt2nd_TMVA);
   reader_trgmu->AddVariable("kaonPt1st", &kaonPt1st_TMVA);
   reader_trgmu->AddVariable("kaonPt2nd", &kaonPt2nd_TMVA);
   reader_trgmu->AddVariable("jpsiPt", &jpsiPt_TMVA);
   reader_trgmu->AddVariable("phiPt", &phiPt_TMVA);
   reader_trgmu->AddVariable("bsPt", &bsPt_TMVA);
   reader_trgmu->AddVariable("mudR", &mudR_TMVA);
   reader_trgmu->AddVariable("kaondR", &kaondR_TMVA);
   reader_trgmu->AddVariable("jpsiPhidR", &jpsiPhidR_TMVA);
   reader_trgmu->AddVariable("svCtxy", &svCtxy_TMVA);
   reader_trgmu->AddVariable("svChi2", &svChi2_TMVA);
   reader_trgmu->AddVariable("svCosine", &svCosine_TMVA);

   TString TMVAmethod_trgmu = "BDT method";
   reader_trgmu->BookMVA("BDT method", "/uscms_data/d3/klau/BtoKll/BParking2018/TMVA_EventSelection/TMVA_Training/dataset/weights/TMVA_DNN_Classification_trgmu_13params_BDT.weights.xml");
   //reader_trgmu->BookMVA("DL_DENSE method", "/uscms_data/d3/klau/BtoKll/BParking2018/TMVA_EventSelection/TMVA_Training/dataset/weights/TMVA_DNN_Classification_trgmu_DL_DENSE.weights.xml");
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	 ...
	 if (preCut_trgmu == true && (muFiredTrgs_lead->at(i) == true || muFiredTrgs_sublead->at(i) == true)) {
	    ...
	    double mvaValue_trgmu = reader_trgmu->EvaluateMVA(TMVAmethod_trgmu);
