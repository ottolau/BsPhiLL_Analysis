...
// METHOD1:
    ...
    fChain->SetBranchStatus("eleSvMass",1);
...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	 ...
	 if (eleSvProb->at(i) < 0.1) cutBasedSelected = false;
	 //if (eleSvLxy->at(i)/eleSvLxyError->at(i) < 3) cutBasedSelected = false;
	 if (eleSvCosAngle->at(i) < 0.999) cutBasedSelected = false;
	 ...
	 */
	 lv_ele_lead.SetPtEtaPhiM(elePt_lead->at(i), eleEta_lead->at(i), elePhi_lead->at(i), elecM);
	 lv_ele_sublead.SetPtEtaPhiM(elePt_sublead->at(i), eleEta_sublead->at(i), elePhi_sublead->at(i), elecM);
	 lv_kaon.SetPtEtaPhiM(kaonEEPt_lead->at(i), kaonEEEta_lead->at(i), kaonEEPhi_lead->at(i), kaonM);
